 ...
 * [MTU(Maximum Transmission Unit)란 무엇인가요?](#mtumaximum-transmission-unit란-무엇인가요)
 * [DNS의 동작 원리를 설명해주세요](#dns의-동작-원리를-설명해주세요)
 * [HTTP와 HTTPS의 차이는 무엇인가요?](#http와-https의-차이는-무엇인가요)
 * [CORS란 무엇이며, 왜 필요한가요?](#cors란-무엇이며-왜-필요한가요)
 * [프록시 서버와 리버스 프록시의 차이는 무엇인가요?](#프록시-서버와-리버스-프록시의-차이는-무엇인가요)
 * [CDN(Content Delivery Network)이란 무엇인가요?](#cdncontent-delivery-network이란-무엇인가요)
 * [NAT(Network Address Translation)이란 무엇인가요?](#natnetwork-address-translation이란-무엇인가요)
 * [웹소켓이란 무엇인가요?](#웹소켓websocket이란-무엇인가요)
 * [세션과 쿠키의 차이는 무엇인가요?](#세션session과-쿠키cookie의-차이는-무엇인가요)
 * [로드밸런싱이란 무엇인가요?](#로드밸런싱load-balancing이란-무엇인가요)
---
## OSI 7계층이란 무엇인가요?
...
---
## 프록시 서버와 리버스 프록시의 차이는 무엇인가요?
프록시 서버는 클라이언트와 서버 사이에 위치하여 요청을 대신 전달하는 서버입니다. 리버스 프록시는 서버 측에 위치해 클라이언트를 대신 받아 서버로 전달합니다.

프록시는 주로 클라이언트 측 캐싱, 접근 제어, 익명화를 위해 사용되고, 리버스 프록시는 [로드밸런싱](#로드밸런싱load-balancing이란-무엇인가요), 보안, SSL 종료 등의 역할을 합니다. 

프록시 서버의 예로는 회사 내부망에서 외부 인터넷 접근 시 프록시서버를 거쳐 로그를 남기고 보안 정책을 적용합니다. 리버스 프록시의 에로는 Nginx가 다수의 백엔드 서버 앞에서 요청을 분산 처리하고, TLS를 종료합니다.

---
## CDN(Content Delivery Network)이란 무엇인가요?
CDN은 전 세계에 분산된 캐시 서버 네트워크로, 사용자와 물리적으로 가까운 서버에서 콘텐츠를 제공하여 지연 시간을 줄이는 기술입니다.

CDN은 이미지, 동영상, JS/CSS  파일 같은 정적 콘텐츠를 캐싱하고, 사용자의 위치에 따라 가장 가까운 엣지 서버에서 제공합니다.

예를 들어 한국 사용자가 미국 서버에 있는 이미지를 요청한다면, CDN은 한국에 위치한 캐시 서버에서 제공하여 응답 속도를 크게 줄입니다. 이는 대역폭 절감, 서버 부하 완화, DDoS 방어에도 기여합니다. Cloudflare, AWS CloudFront, Akamai가 대표적인 CDN 서비스입니다.

---
## NAT(Network Address Translation)이란 무엇인가요?
NAT는 사설 IP 주소를 공인 IP 주소로 변환하여 인터넷과 통신할 수 있게 하는 기술입니다.

IPv4 주소 부족 문제를 해결하고, 내부 네트워크 구조를 외부에 노출하지 않아 보안성도 높아집니다.

---
## 웹소켓(WebSocket)이란 무엇인가요?
웹소켓은 클라이언트와 서버 간 양뱡향 통신을 지원하는 프로토콜입니다

HTTP와 달리 연결을 유지하며, 실시간 데이터 교환이 가능합니다. 채팅, 주식 시세, 게임 등에 사용됩니다.

웹소켓은 HTTP Handshake로 연결을 시작한 뒤, TCP 연결을 유지하며 양방향 메시지를 주고받습니다. 이를 통해 HTTP Polling 대비 대역폭 절감과 지연 시간 감소 효과가 있습니다. 

---
## 세션(Session)과 쿠키(Cookie)의 차이는 무엇인가요?
세션은 서버 측에서 저장되는 사용자 상태 정보이고, 쿠키는 클라이언트 측에 저장되는 작은 데이터입니다.

쿠기는 브라우저가 서버 요청시 자동 전송되며, 세션은 서버 메모리나 저장소에 보관됩니다.

세션은 보안성이 높지만 서버 부하를 증가시킬 수 있고, 쿠키는 클라이언트에 저장되므로 용량 제한과 보안 취약점이 있습니다. JWT 기반 인증은 세션을 대체하여 서버 부하를 줄이는 방법 중 하나입니다.

---
## 로드밸런싱(Load Balancing)이란 무엇인가요?
로드밸런싱은 다수의 서버로 네트워크 트래픽을 분산하는 기술입니다.

트래픽 부하를 분산해 성능과 가용성을 높이고, 특정 서버 장애 시에도 서비스가 지속됩니다. 로드밸런싱 알고리즘으로 라운드 로빈, 최소 연결, IP 해시 등을 사용합니다.
